...
@section('css')
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">
                    <i class="fa fa-file"></i> Lost Item List
                </li>
            </ol>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <div class="col-md-3 col-lg-3 pull-right">
            {!! Form::open(array('action' => 'LostController@index','method'=>'GET')) !!}
        ...
        <div class="col-lg-10 col-md-10 col-sm-10">
            <button class="button success loading-pulse" onclick="window.location.href = 'lost/print'"><i class="fa fa-print"></i> Print</button>
            <button class="button info" onclick="window.location.href = 'lost/excel'"><i class="fa fa-file-excel-o fa-2x"></i></button>
            <table class="table striped hovered cell-hovered border bordered" id="main_table_demo">
                ...
                @foreach($lost_item as $lost_items)
                    <tr>
                        ...
                        <td>{{ $lost_items->item_name }}</td>
                        <td>{{ $lost_items->category }}</td>
                        <td>{{ $lost_items->condition }}</td>
                        <td>{{ $lost_items->id_num }}</td>
                        <td>{{ $lost_items->name }}</td>
                        <td>{{ $lost_items->date_borrow->format('M d Y D') }}</td>
                        <td style="font-weight: bold">{{ $lost_items->date_return->format('M d Y D') }}<br>
                            <span style="font-style:italic;font-size:12px"> {{ $lost_items->date_return->diffForHumans( $lost_items->date_borrow ) }}</span>
                        </td>
                    </tr>
                @endforeach
                </tbody>
